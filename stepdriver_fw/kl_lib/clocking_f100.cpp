...
void Clk_t::UpdateFreqValues() {
    ...
    switch(tmp) {
        ...
        case 0x08: // PLL used as system clock source
            ...
            HsePreDiv = (RCC->CFGR & RCC_CFGR_PPRE1) + 1;
...
// RJ Clock
uint32_t Clk_t::ClockConfigure(Clk_input_t ClkInput,
                    PllMul_t PllMul, AHBDiv_t AHBDiv, APBDiv_t APB1Div, APBDiv_t APB2Div,
                    PLLState_t PLLState) {
    uint32_t StartUpCounter = 0, HSEStatus = 0, HSIStatus = 0;
    if(ClkInput == ciHSE) { // try to setup clock from HSE
        RCC->CR |= ((uint32_t)RCC_CR_HSEON);    // HSE Power On
        RCC->CR |= (uint32_t)RCC_CR_HSEBYP;
        do {
            HSEStatus = RCC->CR & RCC_CR_HSERDY;
            StartUpCounter++;
        } while((HSEStatus == 0) && (StartUpCounter != HSE_STARTUP_TIMEOUT));   // wait while HSE start up
        ...
        if (HSEStatus == (uint32_t)0x01) {
            uint32_t tmp;
            tmp = RCC->CFGR;
            tmp &= (uint32_t)~(RCC_CFGR_PPRE1 | RCC_CFGR_PPRE2 | RCC_CFGR_HPRE);
            tmp |= (AHBDiv << 4);
            tmp |= (APB1Div << 8);
            tmp |= (APB2Div << 11);
            RCC->CFGR = tmp;
        ...
        else { // HSE doesn't Sstart up, lets start from HSI
            return HSEStatus;
        }
    }
    else if (ClkInput == ciHSI) {
        RCC->CR |= ((uint32_t)RCC_CR_HSION);
        do {
            HSIStatus = RCC->CR & RCC_CR_HSIRDY;
        } while((HSIStatus == 0) && (StartUpCounter != HSI_STARTUP_TIMEOUT));
        if ((RCC->CR & RCC_CR_HSIRDY) != RESET) HSIStatus = (uint32_t)0x01;
        else HSIStatus = (uint32_t)0x00;
